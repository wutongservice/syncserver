 ...
 */
package com.borqs.sync.server.contact.engine.source;

import com.borqs.pim.jcontact.JContact;
import com.borqs.sync.server.contact.convert.JContactConverter;
import com.borqs.sync.server.contact.dao.JsonDAOAvroImpl;
import com.borqs.sync.server.contact.manager.JContactManager;
 ...
 */
public class JsonSyncSource extends AbstractSyncSource
    ...
    private JsonManager<JContact> manager;
    private JContactConverter converter;
    private JsonDAO dao;
...
//    private boolean vcardIcalBackend;
    ...
    public void beginSync(SyncContext syncContext) throws SyncSourceException {
        ...
        try {
            ...
            this.dao = new JsonDAOAvroImpl(principal);

            this.converter = new JContactConverter();
            this.manager = new JContactManager(dao, converter);
...
//                vcardIcalBackend = false;
            ...
            manager.beginSync(username, since, AlertCode.getAlertDescription(syncMode), serverTimeZoneID);
            ...
            throw new SyncSourceException("beginSync failed on contact RPC.", e);
        } catch (NotFoundException e){
            throw new SyncSourceException("beginSync failed on RPC lookup.", e);
        }
    }
    ...
    public SyncItem addSyncItem(SyncItem syncItem) throws SyncSourceException {
        ...
        try {
            ...
            JContact contact = convert(content, contentType);
            ...
            // duplication.
            JContact contactTwin = convert(content, contentType);
            JsonItem<JContact> contactForSearch = new JsonItem<JContact>();
...
//            } else {
                ...
                JsonItem<JContact> contactItem = new JsonItem<JContact>();
...
//            } else {
                ...
                JsonItem<JContact> contactItemUpdated = manager.updateExtendedItem(sessionID, contactItem, since);
...
//            }
    ...
    public boolean mergeSyncItems(SyncItemKey syncItemKey, SyncItem syncItem)
        ...
        try {
            ...
            JContact clientContact = convert(content, contentType);
            JsonItem<JContact> clientContactItem = new JsonItem<JContact>();
...
//            } else {
                ...
                JsonItem<JContact> serverContactItem = manager.getExtendedItem(sessionID, serverItemKey);
...
//
//            } else {
                JsonItem<JContact> contactItem = manager.getExtendedItem(sessionID, guid);
                if (contactItem != null) {
                    char status = ' ';
                    // @todo
                    //status = contactItem.getState().charAt(0);
...
//            }
     ...
     */
    private JContact convert(String content, String contentType) throws Exception {
...
//        }
        if(contentType!=null && !"".equals(contentType)){
            contact = JContact.fromJsonString(content);
            return contact;
        }
     ...
     * @param status
     * @throws Exception if the content type is wrong or any problem
     *                         occurs while creating a new SyncItem
     * @return a newly created SyncItem object
     */
    private SyncItem createSyncItem(String id, JContact contact, char status)
            throws Exception {
     ...
     * @param contact
     * @param contentType
     * @throws Exception if the contentType is wrong or the conversion
     *                         attempt doesn't succeed.
     * @return the result in the required format
     */
    private String convert(JContact contact, String contentType)
            throws Exception {
