...
public class JsonAuthenticatorManagerImpl implements JsonAuthenticatorManager {
    ...
    public boolean isSyncing(String username,String deviceId) throws DaoException{
        try {
            JsonResponse jsonResponse = authenticatorDAO.isSyncing(username,deviceId);
            exceptionUtil.throwExceptionOnJsonResponseError(jsonResponse);
            exceptionUtil.throwExceptionOnResponseBodyNull(jsonResponse);
            return parseSyncStatus(jsonResponse);
        } catch (org.apache.commons.httpclient.HttpException httpEx) {
            log.error("Failed the connection to the Json backend", httpEx);
            throw new DaoException(httpEx.getMessage(), httpEx);
        } catch (IOException ioEx) {
    ...
    public void enterSyncEndStatus(String username) throws DaoException{
        try {
            JsonResponse jsonResponse = authenticatorDAO.enterSyncEndStatus(username);
            exceptionUtil.throwExceptionOnJsonResponseError(jsonResponse);
        } catch (org.apache.commons.httpclient.HttpException httpEx) {
            log.error("Failed the connection to the Json backend", httpEx);
            throw new DaoException(httpEx.getMessage(), httpEx);
    ...
    private boolean parseSyncStatus(JsonResponse jsonResponse) throws JSONException {
        /**
         * {"data":{"syncing":"1"}}
         */
        log.info("=====parseSyncStatus:" + jsonResponse.getResponseBody());
       JSONObject body = JSONObject.fromObject(jsonResponse.getResponseBody());
        if(body.has("data")){
            JSONObject root = body.getJSONObject("data");
            if(root.has("syncing")){
                log.info("=====parseSyncStatus,syncing");
                return "1".equalsIgnoreCase(root.getString("syncing"));
            }
        }
        log.info("=====parseSyncStatus,not syncing");
        return false;
    }
}
