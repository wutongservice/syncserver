 ...
 */
package com.borqs.sync.server.officer;

import com.borqs.sync.server.officer.dao.JsonAuthenticatorDAOImpl;
...
import com.funambol.json.exception.*;
...
public class BorqsJsonOfficer extends AbstractOfficer
    ...
    private JsonAuthenticatorManagerImpl authenticatorManager;
    ...
    /**
     * @param autoProvisioning the vcardIcalBackend to set
     */
    ...
    public Sync4jUser authenticateUser(Cred credential) {
        ...
        try {
            this.authenticatorManager = new JsonAuthenticatorManagerImpl(new JsonAuthenticatorDAOImpl(),new AuthenticatorRequestConverter());
        } catch (RPCException e) {
            e.printStackTrace();
        } catch (com.borqs.sync.server.rpc.base.naming.NotFoundException e) {
            e.printStackTrace();
        }
    ...
    private boolean isSyncing(String username,String deviceId){
        try {
            return authenticatorManager.isSyncing(username,deviceId);
    ...
    public void unAuthenticate(Sync4jUser user) {
        ...
        try {
            authenticatorManager.enterSyncEndStatus(user.getUsername());
        } catch (DaoException e) {
            log.info("==============enterSyncEndStatus exception:" + e.getMessage());
            e.printStackTrace();
        }
     ...
     * @return
     */
    private JsonAuthResponse authenticate(JsonAuthRequest authRequest) throws DaoException {
        JsonAuthResponse authResponse = null;
        try {
            authResponse = authenticatorManager.login(authRequest);
        } catch (BadRequestException e) {
            authenticatorManager.enterSyncEndStatus(authRequest.getUser());
