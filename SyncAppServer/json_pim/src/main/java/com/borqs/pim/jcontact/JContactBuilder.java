...
public class JContactBuilder {
    ...
    public String createJson() {
        ...
        try {
            ...
            mContact.put(Tags.TEL, isEmpty(mTel)?null:mTel);
            mContact.put(Tags.EMAIL, isEmpty(mEMail)?null:mEMail);
            mContact.put(Tags.ADDR, isEmpty(mAddress)?null:mAddress);
            mContact.put(Tags.ORG, isEmpty(mOrganization)?null:mOrganization);
            mContact.put(Tags.IM, isEmpty(mIM)?null:mIM);
            mContact.put(Tags.WEBPAGE, isEmpty(mWebpage)?null:mWebpage);
            mContact.put(Tags.XTAG, isEmpty(mXTags)?null:mXTags);
        } catch (JSONException e) {
            e.printStackTrace();
            return null;
        }
